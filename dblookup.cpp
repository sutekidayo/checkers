...
*/
	...
	if(blockpointer[uniqueblockid] != NULL)
		...
		if(fread(diskblock,1024,1,dbfp[dbpointer->fp]) == 0)
		{
			printf("\nFailed to read block with ID %i",uniqueblockid);
		}
...
int db_init(int suggestedMB, char out[256])
	...
	for(n=2;n<SPLITSIZE;n++)
		{
		sprintf(dbname,DB_PATH "db%i.idx",n);
		fp = fopen(dbname,"rb");
		if(fp)
			{
			pieces=n;
			...
			for(bk=0;bk<=nb;bk++)
				...
				for(wk=0;wk<=nw;wk++)
					...
					if(bm+bk==wm+wk && wk>bk)
						continue;
					sprintf(dbname,DB_PATH "db%i_%i%i%i%i.cpr",bm+bk+wm+wk,bm,bk,wm,wk);
		...
		for(nb=maxpieces-maxpiece;nb<=maxpiece;nb++)
			...
			for(bk=0;bk<=nb;bk++)
				...
				for(wk=0;wk<=nw;wk++)
					...
					sprintf(dbname,DB_PATH "db%i_%i%i%i%i.idx",bm+bk+wm+wk,bm,bk,wm,wk);
					sprintf(out,"parsing %s",dbname);
					fp = fopen(dbname,"rb");
					if(fp != NULL)
						{
	...
	blockpointer = new unsigned char *[memsize];
	...
	for(n=2;n<SPLITSIZE;n++)
		{
		sprintf(dbname, DB_PATH "db%i.cpr",n);
...
static int preload(char out[256])
	...
	for(n=2;n<SPLITSIZE;n++)
		...
		while(!feof(fp))
			...
			if(fread(diskblock,1024,1,fp) ==0)
			{
				sprintf(out,"\nError reading block %i",cachepointer);
			}
